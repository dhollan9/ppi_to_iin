...
void modify_network_only(int nwhole, int ninterface, int *numpartners, int **Speclist, Protein *wholep, ppidata *ppi, int *p_home, constrainParms &plist,  ofstream &globalfile, int **ehome, int **Epred, int **templist, int *tmppartners, Protein *wholetemp, int * nbor, int *Adj, ofstream &idfile, ofstream &matchfile)
{
  ...
  double ksi = plist.alpha;
  double mu = plist.mu;
  double gamma=plist.gamma;
...
//Read in pmf file
  ...
  //Create output file
  char fname[300];
  sprintf(fname,"OptNet_beta%g.mu%g.ksi%g.omega%g.zeta%g.Temp%g.fs%d.out", gamma, mu, ksi, omega, zeta, plist.MCtemp, plist.flagsplit);
  ...
  if(fitnessflag==0) 
    network_fit=mc_fit4(ninterface, grid_cofs, clocals, ksi, gamma, nwhole, wholep, mu, numpartners, edgediff);
  else if(fitnessflag==1) //Penalize absolute number of interfaces
    network_fit=mc_fit5(ninterface, grid_cofs, clocals, ksi, gamma, nwhole, wholep, mu, numpartners, edgediff);
  else //Penalize absolute number of interfaces & self loops on hubs
    network_fit=mc_fit6(ninterface, grid_cofs, clocals, ksi, gamma, nwhole, wholep, mu, numpartners, edgediff, Speclist, zeta);
  ...
  cout << " Nentavg: "<<plist.Nentavg<<endl;
  ...
  for(t=1;t<Nit+1;t++){
    ...
    */
	...
	clocals[i]=cluster_cof_mod(i,ntmpinterface,tmppartners,templist);
      ...
      if(fitnessflag==0)
	network_fit=mc_fit4(ntmpinterface, grid_cofs, clocals, ksi, gamma, nwhole, wholetemp, mu, tmppartners, omega*(numedge-PPIedge));
      else if(fitnessflag==1)
	network_fit=mc_fit5(ntmpinterface, grid_cofs, clocals, ksi, gamma, nwhole, wholetemp, mu, tmppartners, omega*(numedge-PPIedge));
      else
	network_fit=mc_fit6(ntmpinterface, grid_cofs, clocals, ksi, gamma, nwhole, wholetemp, mu, tmppartners, omega*(numedge-PPIedge),templist,zeta);
    ...
    if(t%netwrite==0){
      ...
      cout <<"gamma, mu"<<gamma<<", "<<mu<<" iter: "<<t<<" ninterface "<<ninterface<<" network fitness: "<<oldfit<< " avg ratio: "<<avgratio<<" network only fitness: "<< " nsquares: " << nsquares << endl;
	...
	cout <<"gamma, mu, ksi "<<gamma<<", "<<mu<<", "<<ksi<<" iter: "<<t<<" ninterface: "<<ninterface<<" nedge: " << numedge << " nsquares: " << nsquares<<" Alpha (degree dist): " << degalpha <<" Largest module size: " << giant << " network fitness: "<<oldfit<< " avg ratio: "<<avgratio << endl;
	write_ppi(nwhole, wholep);
	write_speclist(ninterface, numpartners, Speclist);
  ...
  //Find mean stats
    ...
    cout <<"gamma, mu, alpha, omega: "<<gamma<<", "<<mu<< ", " << ksi << ", " << omega<< " Acceptance: "<<acc*1.0/(1.0*neq)<<"  decreased and accepted: "<<decacc*1.0/(dec*1.0)<<" decreased: "<<dec*1.0/(1.0*neq)<<endl;
    for(i=0;i<nmove;i++)
